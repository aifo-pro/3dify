    ...
    <article class="relative pb-16 pt-6 sm:pt-10">
        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <nav class="flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-zinc-500 sm:text-sm" aria-label="Breadcrumb">
                ...
                @endif
                <span class="text-zinc-600" aria-hidden="true">›</span>
                <span class="max-w-[min(100%,14rem)] truncate font-medium text-zinc-300 sm:max-w-md">{{ $post->localized_title }}</span>
            </nav>

            {{-- Title block --}}
            <header class="mx-auto mt-8 max-w-4xl text-center lg:mt-10 lg:text-left">
                <div class="flex flex-wrap justify-center gap-2 lg:justify-start">
                    @forelse($post->categories as $category)
                        <a href="{{ route('blog.category', $category) }}" class="inline-flex items-center rounded-full border border-emerald-400/35 bg-emerald-400/10 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] text-emerald-200 transition hover:border-emerald-300/50 hover:bg-emerald-400/15">{{ $category->localized('name') }}</a>
                    @empty
                        <span class="inline-flex items-center rounded-full border border-white/10 bg-white/[0.06] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.16em] text-zinc-400">3Dify Blog</span>
                    @endforelse
                </div>
                <h1 class="mt-5 text-3xl font-bold leading-[1.15] tracking-tight text-white sm:text-4xl sm:leading-tight lg:text-5xl lg:leading-[1.1]">{{ $post->localized_title }}</h1>
                @if($post->localized_excerpt)
                    <p class="mx-auto mt-5 max-w-3xl text-base leading-relaxed text-zinc-400 sm:text-lg lg:mx-0">{{ $post->localized_excerpt }}</p>
                @endif
            </header>

            {{-- Meta + share --}}
            <div class="mx-auto mt-8 flex max-w-4xl flex-col gap-6 border-b border-white/10 pb-8 lg:mx-0 lg:max-w-none lg:flex-row lg:items-center lg:justify-between lg:pb-10">
                <div class="flex flex-wrap items-center justify-center gap-x-6 gap-y-4 text-sm text-zinc-400 lg:justify-start">
                    <div class="flex items-center gap-3">
                        <span class="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-emerald-400/40 bg-emerald-400/15 text-sm font-bold text-emerald-100" aria-hidden="true">{{ $authorInitial }}</span>
                        <div class="leading-tight">
                            <p class="text-xs font-semibold uppercase tracking-wider text-zinc-500">{{ __('blog.author_label') }}</p>
                            <p class="font-semibold text-white">{{ $authorName }}</p>
                        </div>
                    </div>
                    <div class="hidden h-8 w-px bg-white/10 sm:block" aria-hidden="true"></div>
                    <div class="flex items-center gap-2">
                        <svg class="h-4 w-4 shrink-0 text-emerald-400/80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
                    ...
                    <div class="flex items-center gap-2">
                        <svg class="h-4 w-4 shrink-0 text-emerald-400/80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                        <span class="font-medium text-zinc-300">{{ number_format($post->views) }} {{ __('blog.views') }}</span>
                    </div>
                    <div class="flex items-center gap-2">
                        <svg class="h-4 w-4 shrink-0 text-emerald-400/80" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                        <span class="font-medium text-zinc-300">{{ __('blog.reading_time', ['count' => $readMinutes]) }}</span>
                    </div>
                </div>
                <div class="flex flex-wrap items-center justify-center gap-3 lg:justify-end">
                    ...
                    <div class="flex items-center gap-2">
                        <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode($post->url) }}" target="_blank" rel="noopener noreferrer" class="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/[0.05] text-zinc-300 transition hover:border-emerald-400/30 hover:bg-emerald-400/10 hover:text-emerald-100" title="{{ __('blog.share_facebook') }}" aria-label="{{ __('blog.share_facebook') }}">
                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                        </a>
                        <a href="https://t.me/share/url?url={{ urlencode($post->url) }}&text={{ urlencode($post->localized_title) }}" target="_blank" rel="noopener noreferrer" class="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/[0.05] text-zinc-300 transition hover:border-emerald-400/30 hover:bg-emerald-400/10 hover:text-emerald-100" title="{{ __('blog.share_telegram') }}" aria-label="{{ __('blog.share_telegram') }}">
            ...
            @if($post->cover_url)
                <div class="mx-auto mt-10 max-w-5xl">
                    <img src="{{ $post->cover_url }}" alt="{{ $post->localized('cover_alt') ?: $post->localized_title }}" width="1200" height="630" loading="eager" fetchpriority="high" class="aspect-[1200/630] w-full rounded-3xl border border-white/10 bg-zinc-950 object-cover shadow-2xl shadow-black/40">
                </div>
            @endif

            @if($hasToc)
                <nav class="mx-auto mt-8 max-w-3xl rounded-2xl border border-white/10 bg-white/[0.03] p-5 lg:hidden" aria-label="{{ __('blog.toc') }}">
                    <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-emerald-300">{{ __('blog.toc') }}</p>
                    <ol class="mt-4 grid list-none gap-2.5 p-0 text-sm">
                        @foreach($toc as $i => $item)
                            <li>
                                <a href="#{{ $item['id'] }}" class="flex gap-2 text-zinc-400 transition hover:text-emerald-100 {{ $item['level'] === 3 ? 'pl-4' : '' }}">
                                    <span class="w-6 shrink-0 font-mono text-xs text-emerald-400/80">{{ str_pad((string) ($i + 1), 2, '0', STR_PAD_LEFT) }}.</span>
            ...
            <div @class([
                'mt-10 grid gap-10 lg:mt-14 lg:items-start',
                'lg:grid-cols-[13rem_minmax(0,1fr)_17.5rem]' => $hasToc,
                'lg:grid-cols-[minmax(0,1fr)_17.5rem]' => ! $hasToc,
            ])>
                @if($hasToc)
                    <aside class="hidden lg:block">
                        <nav class="sticky top-28 rounded-2xl border border-white/10 bg-gradient-to-b from-white/[0.06] to-white/[0.02] p-5 shadow-lg shadow-black/20" aria-label="{{ __('blog.toc') }}">
                            <p class="text-[11px] font-bold uppercase tracking-[0.2em] text-emerald-300">{{ __('blog.toc') }}</p>
                            <ol class="mt-4 grid list-none gap-3 p-0 text-sm">
                                @foreach($toc as $i => $item)
                                    <li>
                                        <a href="#{{ $item['id'] }}" class="flex gap-2 border-l-2 border-transparent py-0.5 pl-3 text-zinc-400 transition hover:border-emerald-400/60 hover:text-emerald-100 {{ $item['level'] === 3 ? 'pl-6' : '' }}">
                                            <span class="w-6 shrink-0 font-mono text-[11px] text-emerald-400/90">{{ $i + 1 }}.</span>
                                            <span class="leading-snug">{{ $item['text'] }}</span>
                                        </a>
                                    </li>
                                @endforeach
