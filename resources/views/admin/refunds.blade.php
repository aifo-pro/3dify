<x-layouts.admin
    ...
    :description="__('Заявки користувачів на повернення. Підтверджуйте якнайшвидше — це впливає на репутацію маркетплейсу.')"
...
>
    ...
    <div class="mb-5 flex flex-wrap gap-1.5">
        @foreach(['' => __('Усі'), 'pending' => __('Очікують'), 'approved' => __('Затверджені'), 'rejected' => __('Відхилені'), 'refunded' => __('Повернуто')] as $key => $label)
                    ...
                    @forelse($requests as $r)
                        <tr>
                            <td class="px-4 py-3 align-top">
                                <p class="font-mono text-xs text-white">{{ $r->order?->number }}</p>
                                <p class="text-xs text-zinc-500">{{ number_format((float) ($r->order->total ?? 0), 2) }} {{ $r->order->currency ?? '' }}</p>
                                @foreach($r->order?->items ?? [] as $item)
                                    <p class="mt-0.5 text-xs text-zinc-400 truncate max-w-[260px]">— {{ $item->product?->localized('title') ?? $item->product?->title }}</p>
                                @endforeach
                            </td>
                            <td class="px-4 py-3 align-top text-zinc-200">{{ $r->user->name ?? '—' }}<p class="text-xs text-zinc-500">{{ $r->user->email ?? '' }}</p></td>
                            <td class="px-4 py-3 align-top">
